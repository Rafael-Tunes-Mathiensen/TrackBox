    ...
    if (!verifyCSRFToken($csrf_token)) {
        ...
        if (empty($type) || empty($artist) || empty($album_name) || empty($condition_disk) || empty($condition_cover)) {
            ...
            try {
                ...
                $stmt = $pdo->prepare("
                    INSERT INTO disks (
                        user_id, type, artist, album_name, year, label, country_id,
                        is_imported, edition, condition_disk, condition_cover, is_sealed,
                        image_path, is_favorite, observations
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $_SESSION['user_id'], $type, $artist, $album_name, $year, $label, $country_id,
                    $is_imported, $edition, $condition_disk, $condition_cover, $is_sealed,
                    $image_path, $is_favorite, $observations
                ]);
            ...
            <form method="POST" class="disk-form" enctype="multipart/form-data">
                ...
                <div class="form-section">
                    <div class="section-header">
                        ...
                        <p class="section-description">Encontre automaticamente as informações do seu disco</p>
                    </div>
                    <div class="discogs-search-container">
                        <div class="search-input-group">
                            <input type="text" id="discogs_search" class="form-input" placeholder="Digite o nome do artista e álbum...">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <div class="autocomplete-container">
                                <input type="text" id="artist" name="artist" class="form-input autocomplete-input" placeholder="Nome do artista ou banda" required value="<?php echo htmlspecialchars($_POST['artist'] ?? ''); ?>" autocomplete="off">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="album_name" name="album_name" class="form-input" placeholder="Título do álbum" required value="<?php echo htmlspecialchars($_POST['album_name'] ?? ''); ?>">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="year"> <i class="fas fa-calendar"></i> Ano de Lançamento </label>
                            <input type="number" id="year" name="year" class="form-input" placeholder="Ex: 1975" min="1900" max="<?php echo date('Y'); ?>" value="<?php echo htmlspecialchars($_POST['year'] ?? ''); ?>">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="label"> <i class="fas fa-building"></i> Gravadora/Selo </label>
                            <div class="autocomplete-container">
                                <input type="text" id="label" name="label" class="form-input autocomplete-input" placeholder="Nome da gravadora" value="<?php echo htmlspecialchars($_POST['label'] ?? ''); ?>" autocomplete="off">
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        <label class="form-label" for="disk_image"> <i class="fas fa-upload"></i> Selecionar Imagem </label>
                                ...
                                <?php foreach ($countries as $continent => $continent_countries): ?>
                                    <optgroup label="<?php echo htmlspecialchars($continent); ?>">
                                        <?php foreach ($continent_countries as $country): ?>
                                            <option value="<?php echo $country['id']; ?>" <?php echo $country['is_fake_prone'] ? 'data-fake-prone="true"' : ''; ?> <?php echo (isset($_POST['country_id']) && $_POST['country_id'] == $country['id']) ? 'selected' : ''; ?>>
                ...
                <div class="form-section">
                    ...
                    <div class="extras-grid">
                        ...
                        <!-- NOVO CAMPO: OBI -->
                        <div class="extra-item <?php echo (isset($_POST['has_obi'])) ? 'selected' : ''; ?>">
                            <input type="checkbox" id="has_obi" name="has_obi" <?php echo (isset($_POST['has_obi'])) ? 'checked' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="other_extras" name="other_extras" class="form-textarea"
                            placeholder="Descreva outros itens extras que acompanham o disco..."><?php echo htmlspecialchars($_POST['other_extras'] ?? '');
                        ?></textarea>
                    </div>
                </div>
                ...
                <div class="boxset-section" id="boxset_section" style="display: <?php echo (isset($_POST['type']) && $_POST['type'] === 'BoxSet') ? 'block' : 'none'; ?>;">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="special_items" name="special_items" class="form-textarea"
                            placeholder="Descreva os itens especiais incluídos no BoxSet..."><?php echo htmlspecialchars($_POST['special_items'] ?? '');
                        ?></textarea>
                    </div>
                </div>
                ...
                <div class="observations-section">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="observations" name="observations" class="form-textarea"
                            placeholder="Adicione observações sobre raridade, características especiais, etc."><?php echo htmlspecialchars($_POST['observations'] ?? '');
                        ?></textarea>
                    </div>
                </div>
                ...
                <div class="form-navigation">
                    ...
                    <button type="submit" class="btn btn-success">
                        <i class="fas fa-save"></i> Cadastrar Disco
                    </button>
                </div>
            </form>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setupAutocomplete('artist', 'artist_suggestions', 'ajax/autocomplete_artists.php');
        ...
        async function fetchSuggestions(query, endpoint, suggestionsDiv, input) {
            try {
        ...
        async function searchDiscogs(query, page = 1) {
            ...
            try {
                const response = await fetch('ajax/discogs_search.php', {
                    ...
                    body: JSON.stringify({ query: query, type: getSelectedMediaType(), page: page })
                });
                if (!response.ok) {
                ...
                <div class="result-content">
                    ...
                    <div class="result-info">
                        ...
                        <div class="result-formats">
                            ${result.genres ? result.genres.map(g => `<span class="format-tag">${escapeHtml(g)}</span>`).join('') : ''}
                            ${result.styles ? result.styles.slice(0, 3).map(s => `<span class="format-tag style-tag">${escapeHtml(s)}</span>`).join('') : ''}
                        </div>
                    </div>
                </div>
        ...
        function createDiscogsPagination(pagination) {
            const { page, pages, items } = pagination;
            if (pages <= 1) {
                discogsPagination.innerHTML = '';
                return;
            }
            discogsPagination.innerHTML = `
                <button type="button" class="pagination-btn" ${page <= 1 ? 'disabled' : ''} onclick="searchDiscogs('${currentDiscogsQuery}', ${page - 1})">
        ...
        window.useDiscogsData = async function(resultString) {
            try {
                const result = JSON.parse(resultString.replace(/&quot;/g, '"').replace(/&#39;/g, "'"));
                ...
                showNotification('■ Dados preenchidos automaticamente! Verifique e ajuste conforme necessário.', 'success');
        ...
        async function downloadAndSetImage(imageUrl) {
            try {
                ...
                // Adicionar indicador de que a imagem veio do Discogs
                let indicator = imagePreview.querySelector('.auto-filled-indicator');
                if (!indicator) {
        ...
        function markAutoFilledFields() {
            ...
            autoFilledFields.forEach(fieldId => {
                ...
                if (field && field.value) {
                    ...
                    let indicator = field.parentNode.querySelector('.auto-filled-indicator');
                    if (!indicator) {
                        indicator = document.createElement('div');
                        indicator.className = 'auto-filled-indicator';
