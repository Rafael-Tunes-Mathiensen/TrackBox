...
// Agora, sempre exige login para acessar a busca
// Conectar ao banco
...
// NOTA: Toda a lógica relacionada a "coleções compartilhadas" e "shared_collection_owner_id" foi removida.
// A busca agora sempre será para o usuário logado.
...
try {
    ...
    // Buscar discos usando a função searchDisks (agora simplificada)
    $search_results = searchDisks($pdo, $_SESSION['user_id'], $filters, $page, $per_page);
    ...
    setFlashMessage('error', 'Erro ao buscar discos: ' . $e->getMessage());
    $disks = [];
    $total_disks = 0;
    $total_pages = 0;
    $countries = [];
    ...
    <div class="container">
        ...
        <div class="page-header">
            <div class="page-title-container">
                ...
                <div>
                    <h1 class="page-title">Minha Coleção</h1>
                    <p class="page-subtitle">Encontre, edite e gerencie seus discos</p>
                </div>
            </div>
        </div>
            ...
            <form method="GET" class="filters-form" id="searchForm">
                <div class="filters-grid">
                    <div class="filter-group">
                        ...
                        <input type="text" id="search" name="search" class="filter-input"
                            placeholder="Artista ou álbum..." value="<?php echo htmlspecialchars($search); ?>">
                    </div>
                    <div class="filter-group">
                        <label class="filter-label" for="type"> <i class="fas fa-compact-disc"></i> Tipo </label>
                    ...
                    <div class="filter-group">
                        <label class="filter-label" for="condition"> <i class="fas fa-star"></i> Condição </label>
                    ...
                    <div class="filter-group">
                        <label class="filter-label" for="country"> <i class="fas fa-globe"></i> País </label>
                ...
                <div class="filters-actions">
                    ...
                    <button type="button" class="btn btn-accent" id="liveSearchToggle">
                        <i class="fas fa-bolt"></i> <span id="liveSearchText">Ativar Busca Instantânea</span>
                    </button>
                </div>
            </form>
            <!-- Filtros Rápidos -->
            <div class="quick-filters">
                <a href="search_disks.php?filter=sealed" class="quick-filter <?php echo $filter === 'sealed' ? 'active' : ''; ?>"> <i
                        class="fas fa-certificate"></i> Lacrados </a>
                <a href="search_disks.php?filter=imported" class="quick-filter <?php echo $filter === 'imported' ? 'active' : ''; ?>"> <i
                        class="fas fa-globe"></i> Importados </a>
                <a href="search_disks.php?type=BoxSet" class="quick-filter <?php echo $type_filter === 'BoxSet' ? 'active' : ''; ?>"> <i
                        class="fas fa-box-open"></i> BoxSets </a>
                <a href="search_disks.php?filter=favorite" class="quick-filter <?php echo $filter === 'favorite' ? 'active' : ''; ?>"> <i
                        class="fas fa-star"></i> Favoritos </a>
            </div>
        </div>
                ...
                <div class="results-info">
                    <h2>
                        <?php if ($total_disks > 0): ?>
                            <?php echo $total_disks; ?> disco<?php echo $total_disks > 1 ? 's' : ''; ?>
                            encontrado<?php echo $total_disks > 1 ? 's' : ''; ?>
                        <?php else: ?>
                            Nenhum disco encontrado
                        <?php endif; ?>
                    </h2>
                    <?php if (!empty($search) || !empty($type_filter) || !empty($condition_filter) || !empty($country_filter) || !empty($filter)): ?>
                        <p class="search-terms">
                            <?php if (!empty($search)): ?> Busca: <strong><?php echo htmlspecialchars($search); ?></strong> <?php endif; ?>
                            <?php if (!empty($type_filter)): ?> | Tipo: <strong><?php echo $type_filter; ?></strong> <?php endif; ?>
                            <?php if (!empty($condition_filter)): ?> | Condição: <strong><?php echo formatCondition($condition_filter); ?></strong> <?php endif; ?>
                            <?php if ($filter === 'sealed'): ?> | <strong>Lacrados</strong>
                            <?php elseif ($filter === 'imported'): ?> | <strong>Importados</strong>
                            <?php elseif ($filter === 'favorite'): ?> | <strong>Favoritos</strong>
                            <?php endif; ?>
                            <?php if (!empty($country_filter) && $country = array_filter($countries, function($c) use ($country_filter) { return $c['id'] == $country_filter; })) : ?>
                                | País: <strong><?php echo htmlspecialchars(current($country)['country_name']); ?></strong>
                            <?php endif; ?>
                        </p>
                    <?php endif; ?>
                </div>
                <?php if ($total_disks > 0): ?>
                    <div class="results-actions">
                        ...
                        <a href="register_disk.php" class="btn btn-primary">
                            <i class="fas fa-plus-circle"></i> Adicionar Disco
                        </a>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php if (empty($disks)): ?>
                <div class="empty-results">
                    ...
                    <?php if (!empty($search) || !empty($type_filter) || !empty($condition_filter) || !empty($country_filter) || !empty($filter)): ?>
                        ...
                        <a href="search_disks.php" class="btn btn-secondary"> <i class="fas fa-times"></i> Limpar Filtros </a>
                    <?php else: ?>
                    ...
                    <div class="disks-grid" id="disksGrid">
                        <?php foreach ($disks as $disk): ?>
                            <div class="disk-card" data-disk-id="<?php echo $disk['id']; ?>">
                                ...
                                <?php else: ?>
                                    <div class="disk-image-placeholder"> <i class="fas fa-compact-disc"></i> </div>
                                <?php endif; ?>
                                <div class="disk-header">
                                    ...
                                    <div class="disk-badges">
                                        <?php if ($disk['is_sealed']): ?><div class="badge badge-sealed"><i class="fas fa-certificate"></i> Lacrado</div><?php endif; ?>
                                        <?php if ($disk['is_imported']): ?><div class="badge badge-imported"><i class="fas fa-globe"></i> Importado</div><?php endif; ?>
                                        <?php if ($disk['is_favorite']): ?><div class="badge badge-favorite"><i class="fas fa-star"></i> Favorito</div><?php endif; ?>
                                        <?php if ($disk['has_obi']): ?><div class="badge badge-obi"><i class="fas fa-book-reader"></i> OBI</div><?php endif; ?>
                                    </div>
                                </div>
                                <div class="disk-content">
                                    ...
                                    <div class="disk-details">
                                        ...
                                        <?php if ($disk['country_name']): ?><span class="detail-item"><i class="fas fa-globe"></i> <?php echo htmlspecialchars($disk['country_name']); ?></span><?php endif; ?>
                                        <span class="detail-item"><i class="fas fa-hand-sparkles"></i> <?php echo formatCondition($disk['condition_disk']); ?></span>
                                        <span class="detail-item"><i class="fas fa-bookmark"></i> <?php echo formatEdition($disk['edition']); ?></span>
                                    </div>
                                ...
                                <div class="disk-actions">
                                    <a href="disk_details.php?id=<?php echo $disk['id']; ?>" class="btn-action btn-view">
                                        <i class="fas fa-eye"></i> <span>Ver</span>
                                    </a>
                                    <a href="edit_disk.php?id=<?php echo $disk['id']; ?>" class="btn-action btn-edit">
                                        <i class="fas fa-edit"></i> <span>Editar</span>
                                    </a>
                                    <button class="btn-action btn-delete" onclick="confirmDelete(<?php echo $disk['id']; ?>, '<?php echo htmlspecialchars(addslashes($disk['album_name'] . ' - ' . $disk['artist'])); ?>')">
                ...
                <?php if ($total_pages > 1): ?>
                    <div class="pagination">
                        ...
                        <?php if ($page < $total_pages): ?>
                            <a href="?<?php echo http_build_query(array_merge($_GET, ['page' => $page + 1])); ?>"
                                class="pagination-btn"> Próxima <i class="fas fa-chevron-right"></i>
                            </a>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal">
        ...
        <div class="modal-body">
            ...
            <p class="warning-text"> <i class="fas fa-exclamation-triangle"></i> Esta ação não pode ser desfeita! </p>
        </div>
        <div class="modal-footer">
            <button class="btn btn-secondary" onclick="closeDeleteModal()"> <i class="fas fa-times"></i> Cancelar </button>
            <button class="btn btn-danger" id="confirmDeleteBtn"> <i class="fas fa-trash"></i> Excluir Disco </button>
        </div>
    </div>
</div>
...
<style>
    ...
    .quick-filters {
        ...
        flex-wrap: wrap;
        margin-top: 1.5rem;
        /* Adicionado para separar dos botões de ação */
    }
    ...
    .disk-card.list .disk-image,
    .disk-card.list .disk-image-placeholder {
        width: 100px;
        height: 100px;
        margin-right: 1rem;
        margin-bottom: 0;
    ...
    /* NOVO ESTILO: Badge OBI */
    .badge-obi {
        background: rgba(156, 39, 176, 0.1); /* Um tom roxo claro, pode ajustar */
        color: #9C27B0; /* Roxo mais escuro */
    }
    ...
    @media (max-width: 480px) {
        ...
        .disk-actions {
            flex-direction: row;
            /* Mantém os botões lado a lado no mobile para economizar espaço */
            justify-content: space-around;
        }

        .disk-card.list {
            /* Adapta o modo lista para mobile */
            flex-direction: column;
            align-items: center;
        }
...
<script>
    ...
    function showNotification(message, type) {
        ...
        notification.innerHTML = `
            <i class="fas fa-${type === 'success' ? 'check-circle' : 'exclamation-triangle'}"></i> <span>${message}</span>
        `;
        // Adicionar ao body
